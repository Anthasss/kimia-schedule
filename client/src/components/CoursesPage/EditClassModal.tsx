...
interface EditClassModalProps {
  ...
  lecturers: Lecturer[];
  onSave: (updated: CourseClass) => void;
  onClose: () => void;
}
...
export const EditClassModal: React.FC<EditClassModalProps> = ({
  ...
  const [classLetter, setClassLetter] = useState(courseClass.classLetter);
  const [selectedLecturers, setSelectedLecturers] = useState<string[]>([...courseClass.lecturers]);
  ...
  const handleSave = async () => {
    setIsSaving(true);
    onSave({ ...courseClass, classLetter, lecturers: selectedLecturers });
    setIsSaving(false);
  };
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-md w-full p-6 border border-[#c4c6cf] shadow-xl space-y-4">
        <h3 className="font-headline-sm text-[18px] text-[#191c1e]">
          Edit Class {courseClass.classLetter}
        </h3>
